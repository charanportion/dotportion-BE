...
export const handler = async (event) => {
  try {
    logger.info("Received request access event:", JSON.stringify(event));
    ...
    return await accessController.requestAccess(event);
  } catch (error) {
    logger.error("--- UNHANDLED FATAL ERROR in handler ---", error);
    return {
      statusCode: 500,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ error: "A fatal internal server error occured." }),
    };
  }
};
