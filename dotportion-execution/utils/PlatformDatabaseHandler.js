...
export class PlatformDatabaseHandler {
  ...
  async ensureCollectionExists(collectionName, tenant) {
    ...
    if (collections.length === 0) {
      ...
      if (collectionName === 'test') {
        ...
        console.log(`Auto-created 'test' collection for tenant: ${tenant}`);
        return;
      }
      ...
      if (schemaExists) {
        ...
        console.log(`Auto-created collection '${collectionName}' from schema for tenant: ${tenant}`);
      } else {
  ...
  async validateDataAgainstSchema(collectionName, data) {
    try {
      ...
      console.error(`Schema validation error for collection ${collectionName}:`, error);
