...
export class PlatformSchemaHandler {
  ...
  async connect(tenant) {
    try {
      this.logger.info(`Attempting to connect to platform database for tenant: ${tenant}`);
      ...
      this.logger.info(`Successfully connected to platform database: ${dbName}`);
  ...
  async createSchema(tenant, collection, schema) {
    try {
      ...
      this.logger.info(`Created collection in platform database: ${collection}`);
  ...
  async getSchema(tenant, collection) {
    try {
      ...
      this.logger.info(`Getting schema for collection: ${collection} from platform database`);
      ...
      if (!metadataExists) {
        throw new Error("__schema_metadata__ collection does not exist in platform database");
      }
      ...
      if (!result) {
        this.logger.error(`No schema found for collection: ${collection} in platform database`);
        throw new Error(`No schema found for collection: ${collection} in platform database`);
      }
  ...
  async getAvailableCollections(tenant) {
    try {
      ...
      this.logger.error("Error getting available collections from platform database:", error);
      await this.close();
      throw error;
    }
  }
  ...
  async getCollectionParameters(tenant, collection) {
    try {
      ...
      this.logger.error("Error getting collection parameters from platform database:", error);
      await this.close();
      throw error;
    }
  }
    ...
    try {
      ...
      const collectionsWithParams = await Promise.all(
        collections.map(async (collection) => {
          try {
            const parameters = await this.getCollectionParameters(tenant, collection);
            return {
              collection,
              parameters,
              dbType: 'platform'
