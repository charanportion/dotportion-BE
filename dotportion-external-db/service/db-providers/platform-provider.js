...
export class PlatformProvider extends BaseProvider {
  ...
  async connect() {
    if (this.client && this.client.isConnected) {
      this.logger.info("--> Using existing platform MongoDB client connection.");
      return;
    }
  ...
  async listCollections() {
    try {
      await this.connect();
      this.logger.info(`--> Connected to platform database: ${this.db.databaseName}`);
      ...
      this.logger.info(
        `--> Found collections in platform database ${this.db.databaseName}: ${JSON.stringify(
  ...
  async getDocuments(collectionName, { page = 1, limit = 20 }) {
    try {
      ...
      if (collectionName === 'test') {
        const collections = await this.db.listCollections({ name: 'test' }).toArray();
        if (collections.length === 0) {
          await this.db.createCollection('test');
          this.logger.info(`Auto-created 'test' collection for tenant: ${this.tenant}`);
        }
      }
  ...
  async updateDocument(collectionName, documentId, data) {
    try {
      ...
      this.logger.info(
        `Found existing document in platform database: ${JSON.stringify(existingDoc)}`
      );
  ...
  async validateDataAgainstSchema(collectionName, data) {
    try {
      ...
      if (!schemaDoc) {
        this.logger.info(`No schema found for collection ${collectionName}, skipping validation`);
      ...
      if (validationErrors.length > 0) {
        throw new Error(`Platform schema validation failed: ${validationErrors.join(', ')}`);
      }
    } catch (error) {
      if (error.message.includes('Platform schema validation failed')) {
        throw error;
      }
      this.logger.error(`Platform schema validation error for collection ${collectionName}:`, error);
