...
export class PlatformDatabaseHandler {
  ...
  async ensureCollectionExists(collectionName, tenant) {
    ...
    if (collections.length === 0) {
      ...
      if (schemaExists) {
        ...
        logger.info(`Auto-created collection '${collectionName}' from schema for tenant: ${tenant}`);
      } else {
        throw new Error(`Collection '${collectionName}' does not exist and no schema is defined. Please create a schema first using the schema service.`);
      }
    }
  }
  ...
  async hasSchema(collectionName) {
    try {
      ...
      logger.warn(`Error checking schema for collection ${collectionName}:`, error);
      return false;
    }
  }
  ...
  async validateDataAgainstSchema(collectionName, data) {
    try {
      ...
      for (const [fieldName, fieldSchema] of Object.entries(schema)) {
        ...
        if (typeof value === 'number') {
          if (fieldSchema.min !== undefined && value < fieldSchema.min) {
            validationErrors.push(`Field '${fieldName}' must be at least ${fieldSchema.min}`);
          }
          if (fieldSchema.max !== undefined && value > fieldSchema.max) {
            validationErrors.push(`Field '${fieldName}' must be at most ${fieldSchema.max}`);
          }
        }
        ...
        if (fieldSchema.enum && Array.isArray(fieldSchema.enum)) {
          if (!fieldSchema.enum.includes(value)) {
            validationErrors.push(`Field '${fieldName}' must be one of: ${fieldSchema.enum.join(', ')}`);
          }
        }
      }
      ...
      logger.error(`Schema validation error for collection ${collectionName}:`, error);
      return { valid: true }; // Default to valid if schema validation fails
    }
  }

  async executeOperation(operation, collection, resolvedFilter, resolvedData, options) {
    ...
    switch (operation) {
      ...
      case "insertOne":
        ...
        const insertValidation = await this.validateDataAgainstSchema(collection, resolvedData);
        if (!insertValidation.valid) {
          throw new Error(`Schema validation failed: ${insertValidation.errors.join(', ')}`);
        }
      ...
      case "insertMany":
        ...
        for (const doc of resolvedData) {
          const validation = await this.validateDataAgainstSchema(collection, doc);
          if (!validation.valid) {
            throw new Error(`Schema validation failed for document: ${validation.errors.join(', ')}`);
          }
        }
      ...
      case "updateOne":
        ...
        const updateValidation = await this.validateDataAgainstSchema(collection, resolvedData);
        if (!updateValidation.valid) {
          throw new Error(`Schema validation failed: ${updateValidation.errors.join(', ')}`);
        }
      ...
      case "updateMany":
        // Validate update data against schema
        const updateManyValidation = await this.validateDataAgainstSchema(collection, resolvedData);
        if (!updateManyValidation.valid) {
          throw new Error(`Schema validation failed: ${updateManyValidation.errors.join(', ')}`);
        }
